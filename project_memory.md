...
### 15. Fallo de Envío por Validación en Nuevo Formulario Angular de SiGCA (Calificación de Estrellas)
* **Fallo:** Tras la actualización del portal web de SiGCA a una nueva interfaz en Angular (`ng-version="20.3.27"`), el bot fallaba al enviar la solicitud indicando `No se detectó el mensaje de éxito después de enviar. Puede que esté lento o haya fallado.` debido a que la pregunta de evaluación anterior cambió su etiqueta a un título dinámico (`¿Te gustó el almuerzo del <Día> <Fecha>?`) y su componente visual pasó de radios a botones interactivos de estrellas (`button.rq-star`, `[title*='de 5']`, `[role='radiogroup']`). Al omitirse este campo obligatorio, el formulario permanecía en estado `ng-invalid` y Angular bloqueaba el envío del formulario.
* **Solución:** Se integró soporte para campos de calificación (`field_type="rating"`) en `fill_form_field` dentro de [src/bot_engine.py](file:///c:/Users/Administrador/Desktop/python/solicitud_almuerzo_auto/src/bot_engine.py), permitiendo seleccionar estrellas por atributo `title`, índice o radiogroup. Se ampliaron los disparadores de eventos `change`/`input` para reactividad en Angular, se adaptó la detección del título dinámico de la pregunta de evaluación y se enriqueció la verificación de mensajes de confirmación posteriores al envío.

### 16. Ejecución Indeseada de Solicitudes al Registrar Tarea o por Tarea Programada con Bot Inactivo
* **Fallo:** Al registrar la tarea en Windows desde la interfaz gráfica o al dispararse la tarea programada con el bot desactivado, el sistema continuaba ejecutando solicitudes de almuerzo. Esto se debía a: (1) En `_finish_task_action` de [app_gui.py](file:///c:/Users/Administrador/Desktop/python/solicitud_almuerzo_auto/app_gui.py), al registrar la tarea dentro de la ventana horaria se lanzaba inmediatamente un subproceso de solicitud sin verificar si el bot estaba activo; (2) `LunchBot.run_automation()` en [src/bot_engine.py](file:///c:/Users/Administrador/Desktop/python/solicitud_almuerzo_auto/src/bot_engine.py) carecía de comprobación del estado `is_active`; (3) en la CLI, el parámetro `--force-time` anulaba la comprobación de inactividad; (4) no se reconocían posibles banderas de inactividad manuales en `config.json`.
* **Solución:** Se implementó `is_bot_active()` en [src/config.py](file:///c:/Users/Administrador/Desktop/python/solicitud_almuerzo_auto/src/config.py) que unifica la consulta de `status.json` y `config.json`. Se incorporó la validación temprana estricta en `run_automation()` de [src/bot_engine.py](file:///c:/Users/Administrador/Desktop/python/solicitud_almuerzo_auto/src/bot_engine.py), se corrigió el modo CLI para que solo ejecuciones manuales explícitas (`--manual`) puedan ignorar la inactividad, y en `_finish_task_action` se omitió el disparo de subprocesos cuando el bot se encuentra desactivado, mostrando una notificación informativa adecuada.
